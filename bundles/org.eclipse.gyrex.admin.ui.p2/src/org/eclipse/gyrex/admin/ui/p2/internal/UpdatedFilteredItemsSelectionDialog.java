...
//[gyrex]	use new class FilteredItemsSelectionDialogMessages to get messages instead of WorkbenchMessages
//[gyrex]	use P2UiActivator to get image (properties)
//[gyrex]	use P2UiActivator.SYMBOLIC_NAME as plugin id instead of WorkbenchPlugin.PI_WORKBENCH
/**
 ...
 * @since 1.1
 */
public abstract class UpdatedFilteredItemsSelectionDialog extends SelectionStatusDialog {
	...
	protected abstract class AbstractContentProvider {
		 ...
		 * @param itemsFilter
		 *            the filter
		 * @see UpdatedFilteredItemsSelectionDialog.ItemsFilter#matchItem(Object)
		 */
	...
	private class ContentProvider extends AbstractContentProvider implements IStructuredContentProvider, ILazyContentProvider {
		...
		private void checkDuplicates(final IProgressMonitor monitor) {
			synchronized (lastFilteredItems) {
				...
				if (monitor != null) {
					...
					subMonitor.beginTask(FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_cacheRefreshJob_checkDuplicates, 5);
				}
		...
		protected Object[] getFilteredItems(final Object parent, IProgressMonitor monitor) {
			...
			monitor.beginTask(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_cacheRefreshJob_getFilteredElements, ticks);
			if (filters != null) {
...
//		}
		 ...
		 * {@link UpdatedFilteredItemsSelectionDialog.ContentProvider#getFilteredItems(Object, IProgressMonitor)}
		 * .
		 * 
		...
		public void reloadCache(final boolean checkDuplicates, final IProgressMonitor monitor) {
			...
			if (monitor != null) {
				...
				monitor.beginTask(FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_cacheRefreshJob, totalWork);
			}
...
//		}
	...
	private class FilterHistoryJob extends Job {
		...
		public FilterHistoryJob() {
			super(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_jobLabel);
			setSystem(true);
		}
	 ...
	 * (
	 * {@link UpdatedFilteredItemsSelectionDialog.ItemsFilter#isSubFilter(UpdatedFilteredItemsSelectionDialog.ItemsFilter)}
	...
	private class FilterJob extends Job {
		...
		protected IStatus doRun(final GranualProgressMonitor monitor) {
			try {
				...
				cancel();
				return new Status(IStatus.ERROR, P2UiActivator.SYMBOLIC_NAME, IStatus.ERROR, FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_jobError, e);
			}
			return Status.OK_STATUS;
		}
		...
		protected void filterContent(final GranualProgressMonitor monitor) throws CoreException {
			...
			if ((lastCompletedFilter != null) && lastCompletedFilter.isSubFilter(itemsFilter)) {
				...
				monitor.beginTask(FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_cacheSearchJob_taskName, length);
				...
				if (monitor != null) {
					monitor.beginTask(FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_searchJob_taskName, 100);
	...
	private class GranualProgressMonitor extends ProgressMonitorWrapper {
		...
		private String getMessage() {
			...
			if (name == null) {
				...
				message = subName == null ? name : NLS.bind(FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_subtaskProgressMessage, new Object[] { name, subName });
			}
			if (totalWork == 0) {
				return message;
			}

			return NLS.bind(FilteredItemsSelectionDialogMessages.get(display).FilteredItemsSelectionDialog_taskProgressMessage, new Object[] { message, new Integer((int) ((worked * 100) / totalWork)) });
...
//			setOwnerDrawEnabled(provider instanceof IStyledLabelProvider);
		...
		private Image getImage(final Object element) {
			if (element instanceof ItemsListSeparator) {
				return P2UiActivator.getInstance().getImageRegistry().get(P2UiImages.IMG_VIEW_MENU);
			}
...
//		}
		...
		private String getSeparatorLabel(final String separatorLabel) {
			...
			final int imageWidth = P2UiActivator.getInstance().getImageRegistry().get(P2UiImages.IMG_VIEW_MENU).getBounds().width;
...
//			super.update(cell);
	 ...
	 * 
	 * @see UpdatedFilteredItemsSelectionDialog.RefreshJob
	 */
	private class RefreshCacheJob extends Job {
		...
		public RefreshCacheJob() {
			super(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_cacheRefreshJob);
			setSystem(true);
		}
		...
		protected IStatus run(final IProgressMonitor monitor) {
			if (monitor.isCanceled()) {
				return new Status(IStatus.CANCEL, P2UiActivator.SYMBOLIC_NAME, IStatus.CANCEL, EMPTY_STRING, null);
			}

			if (UpdatedFilteredItemsSelectionDialog.this != null) {
// RAP [if] fake context
			...
			return new Status(IStatus.OK, P2UiActivator.SYMBOLIC_NAME, IStatus.OK, EMPTY_STRING, null);
		}
	 ...
	 * </ul>
	 * 
	 * @see org.eclipse.ui.dialogs.UpdatedFilteredItemsSelectionDialog.FilterJob
	 * @see org.eclipse.ui.dialogs.UpdatedFilteredItemsSelectionDialog.RefreshJob
	 * @see org.eclipse.ui.dialogs.UpdatedFilteredItemsSelectionDialog.RefreshCacheJob
	 */
	private class RefreshJob extends UIJob {
		 ...
		 */
		public RefreshJob() {
			super(getParentShell().getDisplay(), FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_refreshJob);
			setSystem(true);
		}
	...
	private class RefreshProgressMessageJob extends UIJob {
		...
		public RefreshProgressMessageJob() {
			super(getParentShell().getDisplay(), FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_progressRefreshJob);
			setSystem(true);
		}
		...
		public IStatus runInUIThread(final IProgressMonitor monitor) {
			...
			if ((progressMonitor == null) || progressMonitor.isDone()) {
				return new Status(IStatus.CANCEL, P2UiActivator.SYMBOLIC_NAME, IStatus.OK, EMPTY_STRING, null);
			}
	...
	private class RemoveHistoryItemAction extends Action {
		...
		public RemoveHistoryItemAction() {
			super(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_removeItemsFromHistoryAction);
		}
...
//		protected abstract void storeItemToMemento(Object item, IMemento memento);
	...
	private class ToggleStatusLineAction extends Action {
		...
		public ToggleStatusLineAction() {
			super(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_toggleStatusAction, IAction.AS_CHECK_BOX);
		}
...
//	private IHandlerActivation showViewHandler; //we don't need this in gyrex
	 ...
	 */
	public UpdatedFilteredItemsSelectionDialog(final Shell shell) {
		this(shell, false);
	}
	 ...
	 */
	public UpdatedFilteredItemsSelectionDialog(final Shell shell, final boolean multi) {
		super(shell);
// RAP [rh] store display to access NLS messages		
		...
		itemsListSeparator = new ItemsListSeparator(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_separatorLabel);
		selectionMode = NONE;
	}
...
//		restoreDialog(getDialogSettings());
	...
	private Label createHeader(final Composite parent) {
		...
		headerLabel.setText(((getMessage() != null) && (getMessage().trim().length() > 0)) ? getMessage() : FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_patternLabel);
...
//		});
	...
	private Label createLabels(final Composite parent) {
		...
		listLabel.setText(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_listLabel);
...
//		});
	...
	private void createViewMenu(final Composite parent) {
		...
		toolItem.setImage(P2UiActivator.getInstance().getImageRegistry().get(P2UiImages.IMG_VIEW_MENU));
		toolItem.setToolTipText(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_menu);
...
//		showViewHandler = service.activateHandler(IWorkbenchCommandConstants.WINDOW_SHOW_VIEW_MENU, handler, new ActiveShellExpression(getShell()));
	 ...
	 *            {@link UpdatedFilteredItemsSelectionDialog.AbstractContentProvider#add(Object, UpdatedFilteredItemsSelectionDialog.ItemsFilter)}
	...
	protected void fillContextMenu(final IMenuManager menuManager) {
		...
		if (selectedElements.size() > 0) {
			removeHistoryItemAction.setText(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_removeItemsFromHistoryAction);
	...
	protected void handleSelected(final StructuredSelection selection) {
		IStatus status = new Status(IStatus.OK, P2UiActivator.SYMBOLIC_NAME, IStatus.OK, EMPTY_STRING, null);
		...
		if (selection.size() == 0) {
			status = new Status(IStatus.ERROR, P2UiActivator.SYMBOLIC_NAME, IStatus.ERROR, EMPTY_STRING, null);
			...
			for (final Iterator<Object> it = items.iterator(); it.hasNext();) {
				...
				if (tempStatus.isOK()) {
					status = new Status(IStatus.OK, P2UiActivator.SYMBOLIC_NAME, IStatus.OK, EMPTY_STRING, null);
				} else {
					status = tempStatus;
	...
	private void refreshDetails() {
		...
		switch (selection.size()) {
			...
			case 1:
				...
				break;
			default:
				details.setInput(NLS.bind(FilteredItemsSelectionDialogMessages.get().FilteredItemsSelectionDialog_nItemsSelected, new Integer(selection.size())));
				break;
		}
...
// RAP [rh] fake context	  
	 ...
	 * @see UpdatedFilteredItemsSelectionDialog#FULL_SELECTION
	 */
	 ...
	 *            initial pattern for the filter
	 * @param selectionMode
	 *            one of: {@link UpdatedFilteredItemsSelectionDialog#NONE},
	 *            {@link UpdatedFilteredItemsSelectionDialog#CARET_BEGINNING},
	 *            {@link UpdatedFilteredItemsSelectionDialog#FULL_SELECTION}
	 */
