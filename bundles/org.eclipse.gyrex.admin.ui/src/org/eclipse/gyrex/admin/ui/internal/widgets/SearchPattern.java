...
public class SearchPattern {
	 ...
	 * is, the source of the search result equals the search pattern. Search
	 * pattern should start from lowerCase char.
	 */
	 ...
	 */
	private static String trimWildcardCharacters(final String pattern) {
	 ...
	 */
	public SearchPattern(final int allowedRules) {
	 ...
	 *         false otherwise
	 */
	private boolean camelCaseMatch(final String pattern, final int patternStart, int patternEnd, final String name, final int nameStart, int nameEnd) {
		if (name == null) {
	 ...
	 */
	private boolean camelCaseMatch(final String pattern, final String name) {
		if (pattern == null) {
			return true; // null pattern is equivalent to '*'
		}
		if (name == null) {
	 ...
	 */
	public boolean equalsPattern(final SearchPattern pattern) {
		return trimWildcardCharacters(pattern.initialPattern).equals(trimWildcardCharacters(initialPattern));
	}
	...
	private void initializePatternAndMatchRule(final String pattern) {
	 ...
	 */
	protected boolean isNameCharAllowed(final char nameChar) {
	 ...
	 * <code>a.isSubPattern(b)</code> is <code>true</code> iff <code>b</code> is
	 * a sub-pattern of <code>a</code>, and not vice-versa. </i>
	 * </p>
	 * 
	 * @param pattern
	 *            pattern to be checked
	 ...
	 */
	public boolean isSubPattern(final SearchPattern pattern) {
		return trimWildcardCharacters(pattern.initialPattern).startsWith(trimWildcardCharacters(initialPattern));
	}
	 ...
	 */
	protected boolean isValidCamelCaseChar(final char ch) {
		return true;
	}
	 ...
	 *         detected.
	 */
	private int validateMatchRule(final String stringPattern, int matchRule) {
